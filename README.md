...
Android also includes a **Study** tab with 24 searchable topic markers and 120 offline sections: explanations, examples, cheatsheets, self-checks and primary references. The syllabus covers Java foundations, OOP, Java 8/9, Spring, Hibernate, Servlets, JSP, CDI, EJB, JMS, ActiveMQ, REST, security, servers, deployment and architecture, plus Java 17/21 modernization notes. Save topics, mark them read and resume the last section; these records are included in progress backups without changing exam scores. Lessons live in `apps/learner/study/` and are embedded in the APK. Reference links require internet. This is independent study material, not an official Accenture syllabus or reproduction of its assessments.

The published web app—including its mobile-browser layout—does not include this reader or its lesson text. To test the Android UI locally in a browser, build an explicit debug harness with `dx build --web --package tutorialz-learner --features study-preview --base-path ''`, serve `target/dx/tutorialz-learner/debug/web/public` on port 8876, then run `node scripts/study-test.mjs`. Never enable `study-preview` in a production web build. `tests/study_content_test.py` validates lesson structure and compiles the complete Java examples against their stated Java versions.
